...
const Checkout = () => {
  const cart = useSelector((state) => state.handle_cart);

  const total = (cart, discount) => {
    const from_cart = cart
      .map((el) => el.price * el.quantity)
      .reduce((acc, el) => acc + el);
    const discount_amount = from_cart - discount;
        ...
        <div className="row g-5">
          <div className="col-md-5 col-lg-4 order-md-last">
            ...
            <ul className="list-group mb-3">
              {cart.length > 0 && cart.map((item) => checkout_item(item))}
              ...
              <li className="list-group-item d-flex justify-content-between">
                <span>Total (USD)</span>
                <strong>$ {cart.length !== 0 && total(cart, 5)} </strong>
              </li>
            </ul>
